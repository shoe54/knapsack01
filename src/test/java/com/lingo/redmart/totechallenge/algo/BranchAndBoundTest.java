...
import com.lingo.redmart.totechallenge.algo.BranchAndBound.Node;

public class BranchAndBoundTest {
	static BranchAndBound bnb;
	...
	public void testMaximizeMyShopping01() {
		...
		bnb.doMaximizeMyShopping(products, t);
	 ...
	 * I also added some of my own manually calculated sets
	 */
	@Test
	public void testMaximizeMyShopping04() {
		...
		bnb.doMaximizeMyShopping(products, t);
		assertEquals(TestUtil.setOfProducts(p1,p19,p21,p11), t.getProducts()); // total price should be 290
	...
	/**
	 * Test using data set from the Foundations of Algorithms Using Java Pseudocode book
	 */
	@Test
	public void testMaximizeMyShopping06() {
		Tote t = new Tote(1, 16, 1); // volume: 16
		List<Product> products = new ArrayList<>();

		Product p1 = new Product(1, 40, 2, 1, 1, 10); // Volume: 2
		Product p2 = new Product(2, 30, 1, 5, 1, 10); // Volume: 5
		Product p3 = new Product(3, 50, 1, 1, 10, 10); // Volume: 10
		Product p4 = new Product(4, 10, 1, 5, 1, 10); // Volume: 5 
		...
		bnb.doMaximizeMyShopping(products, t);
		assertEquals(TestUtil.setOfProducts(p1,p3), t.getProducts());
	}
	...
	public void testBound01() {
		...
		result = bnb.bound(Node.ZERO, t, products);
		assertEquals(result, new PriceWeightTuple<Double>(5.0d, 5.0d));
	}

	@Test
	public void testBound02() {
		Tote t;
		List<Product> products = new ArrayList<>();
		PriceWeightTuple<Double> result;
		...
		t = new Tote(10, 1, 1); // volume: 10
		result = bnb.bound(Node.ZERO, t, products);
		assertEquals(152.6923, result.getPrice().doubleValue(), 0.0001);
		assertEquals(10.7692, result.getWeight().doubleValue(), 0.0001);

		t = new Tote(1, 50, 1); // volume: 50
		result = bnb.bound(Node.ZERO, t, products);
